...
$os                     = "ubuntu"
$num_instances          = 1
$instance_name_prefix   = "mattermost"
$vm_memory              = 2048
$vm_cpus                = 1
$subnet                 = "10.0.1.6" # For 10.0.1.6X
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "mattermost" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"],
            "postgresql" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"]
          }
        end
      end
    end
